...
;------------------------------------------------------------------------------
RX:
waitch:     in a,($80)    ; read status byte from ACIA    
            bit 0,a       ; test if character available
            jr z,waitch   ; busy loop until it is
            ld a,$56      ; de-assert RTS & disable RX interrupt
            out ($80),a   ; set updated status on ACIA
            in a,($81)    ; retrieve received character into A
            ret           ; return

;------------------------------------------------------------------------------
TX:         push     af              ; Store character
...
RESET:  ld hl,RAMEND
        ...
        ld a, 0x96   ; initialize ACIA
        out (0x80),a
        ld de,1      ; do reset if COLD returns
        jp COLD      ; enter top-level Forth word

; Memory map:
;   0-2000h     Forth kernel = start of 
;     ? h       Forth dictionary (user RAM)
;   EM-280h     Terminal Input Buffer, 128 bytes
...
enddict equ 0x2000       ; user's code starts here
