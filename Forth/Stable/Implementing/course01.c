 ...
 *
 * We need a datastack and a dictionary, no string handling
 */
...
static int sp_base[DATA_STACK_SIZE], *sp_end=sp_base+DATA_STACK_SIZE;
static int *sp=sp_base-1;
...
static void sp_push(int value) {
	if(sp==sp_end) terminate("Data stack overflow");
	*++sp=value;
}
static int sp_pop(void) {
